...
if [[ -n "$VALIDHOSTNAME" ]]; then
  ...
  /root/.oldroot/nfs/install/installimage -a -p /boot:ext3:1G,lvm:vg0:all -v vg0:root:/:ext4:20G,vg0:swap:swap:swap:8G -r yes -l 1 -b grub -d nvme0n1,nvme1n1 -i /root/.oldroot/nfs/install/../images/Debian-1011-buster-64-minimal.tar.gz -x hetzner-postinstall.sh -n ${SERVERNAME}
fi
